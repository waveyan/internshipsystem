...
password VARCHAR (10) NULL,
inforCheck int DEFAULT 0  NULL comment "实习信息审核提示（0无提示，1提示）",
jourCheck int DEFAULT 0  NULL comment "实习日志审核提示（0无提示，1提示）",
sumCheck int DEFAULT 0   NULL comment "实习总结审核提示（0无提示，1提示）",
roleId int DEFAULT 0  NULL comment "角色ID，外键对应Role表的roleId"
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
comCheck int DEFAULT 0 NULL comment "审核状态（0表示待审核，1表示被退回修改，2表示审核通过）"
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
icheckTeaId varchar(8) NULL comment "审核教师工号",
opinion VARCHAR (250) NULL comment'审核意见',
internCheck int DEFAULT 0  NULL comment "审核状态（0表示待审核，1表示被退回修改，2表示审核通过）",
icheckTime datetime  NULL comment "审核时间",
...
sumCheck int DEFAULT 0 NULL comment "审核状态（0表示待审核，1表示被退回修改，2表示审核通过）",
scheckTeaId varchar(8) NULL comment "审核教师工号",
scheckTime datetime NULL comment "审核时间",
KEY sumFK(stuId),
PRIMARY KEY (sumId),
...
jourCheck int DEFAULT 0 NULL comment "审核状态（0表示待审核，1表示被退回修改，2表示审核通过）",
jcheckTeaId varchar(8) NULL comment "审核教师工号",
jcheckTime datetime NULL comment "审核时间",
KEY jourFK1(stuId),
KEY jourFK2(comId),
